...
if (isset($data['rescuer_name'])) {
    ...
    if (isset($data['selected_products'])) {
        ...
        // Loop through selected products to remove them from the cargo
        foreach ($selectedProducts as $productName => $quantity) {
            // Replace the product and its quantity with an empty string
            $currentCargo = str_replace(", $productName", '', $currentCargo);
        }
