...
if (isset($data['id'])) {
    ...
    // Update the 'accepted' field in the 'announcements_cityzen' table
    $sql = "UPDATE announcements_cityzen SET accepted = 'not_expected', date_of_comp = ? WHERE id = ?";

    // Prepare and execute the SQL statement
    $stmt = $conn->prepare($sql);
    
    // Bind parameters
    $stmt->bind_param('si', $dateOfResponse, $id); // 'i' stands for integer type
    $stmt->execute();
