...
if (isset($data['id'])) {
    ...
    // Update the 'accepted' and 'date_of_response' fields in the 'help_offering' table
    $sqlHelpOffering = "UPDATE help_offering SET accepted = 'waiting', date_accepted = ?, rescuer_name = ? WHERE id = ?";

    // Prepare and execute the SQL statement for help_offering table
    $stmtHelpOffering = $conn->prepare($sqlHelpOffering);
    $stmtHelpOffering->bind_param('ssi', $dateOfResponse, $rescuerName, $id);
    $stmtHelpOffering->execute();

    // Check if the update was successful for help_offering table
    if ($stmtHelpOffering->affected_rows <= 0) {
        // Return error message if the update fails
        echo "Error updating status in help_offering table";
        $stmtHelpOffering->close();
        $conn->close();
        return;
    }
    else if ($stmtHelpOffering->affected_rows > 0) {
    ...
    // Close the statement for help_offering table
    $stmtHelpOffering->close();
