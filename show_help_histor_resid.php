...
$sql = "SELECT `id`, `id_original`, `accepted`, `username` FROM `help_offering`";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc() ) {
        if ($row["username"] == $username) {
            ...
            echo "<button id='btn1" . $row["id_original"] . "' onclick='buttonClicked1(" . $row["id_original"] . ")'>Delete</button>";
            echo "<br><br>";
        }
    }
} else {
